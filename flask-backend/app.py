...
def unauthorized_response(callback):
    return jsonify({
        ...
        'message': 'Missing or invalid JWT token in the Authorization header.'
    }), 401
...
SARVAM_API_KEY = "cf07b2de-ce8b-40b8-ae96-b26faeef4acd"  # Replace with your actual Sarvam API key
...
def validate_audio_file(audio_file_path):
    """Validate the audio file format and ensure it exists."""
    if not os.path.exists(audio_file_path):
        raise MultilingualError(f"Audio file not found: {audio_file_path}")

    file_extension = audio_file_path.split('.')[-1].lower()
    if file_extension not in SUPPORTED_AUDIO_FORMATS:
        raise MultilingualError(
            f"Unsupported audio format: {file_extension}. Supported formats: {SUPPORTED_AUDIO_FORMATS}")

    file_size_mb = os.path.getsize(audio_file_path) / (1024 * 1024)
    if file_size_mb > 10:
        raise MultilingualError(f"Audio file too large: {file_size_mb:.2f}MB. Maximum allowed: 10MB")
...
def convert_audio_to_wav(audio_file_path):
    ...
    if file_extension != "wav":
        try:
            ...
            wav_path = audio_file_path.replace(f".{file_extension}", "_converted.wav")
            audio.export(wav_path, format="wav", parameters=["-ac", "1", "-ar", "16000"])  # Mono, 16 kHz
            logger.debug(f"Converted audio to WAV: {wav_path}")
            return wav_path
        except Exception as e:
            raise MultilingualError(f"Audio conversion failed: {str(e)}")
...
def speech_to_text(audio_file_path, language_code="ta-IN"):
    ...
    try:
        ...
        # Additional validation with librosa
        try:
            audio, sr = librosa.load(audio_file_path, sr=None)
            duration = librosa.get_duration(y=audio, sr=sr)
            if duration < 1.0:
                raise MultilingualError("Audio duration is too short (less than 1 second)")
            logger.debug(f"Audio file validated - Duration: {duration} seconds, Sample Rate: {sr}")
        except Exception as e:
            logger.error(f"Failed to validate audio file: {str(e)}")
            raise MultilingualError(f"Invalid audio file: {str(e)}")

        # Convert to WAV if needed
        temp_audio_path = convert_audio_to_wav(audio_file_path)

        # Determine the MIME type dynamically
        mime_type, _ = mimetypes.guess_type(temp_audio_path)
        if not mime_type:
            extension = Path(temp_audio_path).suffix.lower()
            if extension == '.wav':
                mime_type = 'audio/wav'
            else:
                raise MultilingualError(f"Unsupported audio file format after conversion: {extension}")
        ...
        with open(temp_audio_path, "rb") as audio_file:
            ...
            logger.debug(f"Sending STT request - File: {temp_audio_path}, Language: {language_code}, MIME Type: {mime_type}")
        ...
        if response.status_code == 200:
            result = response.json()
            logger.debug(f"Sarvam STT raw response: {result}")
            
            # Try different possible field names for the transcription
            transcribed_text = result.get('transcription', '') or result.get('text', '') or result.get('result', '')
        ...
        logger.error(f"Network error during STT: {str(e)}")
        raise MultilingualError(f"Network error during STT: {str(e)}")
    except Exception as e:
        logger.error(f"Error in speech_to_text: {str(e)}")
        raise MultilingualError(f"Error in speech_to_text: {str(e)}")
    finally:
        # Clean up temporary converted file if it exists
        if 'temp_audio_path' in locals() and temp_audio_path != audio_file_path and os.path.exists(temp_audio_path):
...
def get_conversational_chain():
    ...
    - *Greeting & Acknowledgment*:  
      Greet the user by name (if available) or generically and acknowledge the query briefly. This should be a standalone paragraph before any section, followed by a blank line.  
    ...
    Constraints:  
    - Always start sections with '- **Section Name:*' on its own line (e.g., '- **Eligibility Criteria:*'), followed by a blank line before the content.  
    - Use '* - ' for bullet points under sections, with each bullet on a new line.  
    - Use numbered lists (e.g., '1. ') for required information, with each item on a new line.  
    - Avoid markdown symbols like '*' or '**' within the content of sentences (e.g., do not bold individual words in sentences).  
    ...
    - Ensure there are no extra spaces or content on the same line as section headings.

    Answer:  
    (Generate a structured response following the guidelines, ensuring section headings use '- **Section Name:*' on a separate line, bullet points use '* - ', and there are no extra spaces or content on the same line as section headings.)"""

    model = ChatGoogleGenerativeAI(model="gemini-1.5-pro", temperature=0.1, google_api_key=GOOGLE_API_KEY)
...
def format_response_to_html(text):
    ...
    print(f"Raw text to format: {text}")  # Debug log
    lines = text.split('\n')
    html_lines = []
    in_list = False
    ...
    while i < len(lines):
        ...
        # Match section headings like '- **Section Name:*' or '- **Section Name:**' even if followed by content
        ...
        if section_match:
            ...
            print(f"Found section: {section_title}")  # Debug log

            # If there's content on the same line (e.g., '- **Profile Issue:** Your date...')
            if section_match.group(2):
                next_line = section_match.group(2).strip()
                if next_line:
                    # Check if the content looks like a bullet point (even without '* - ')
                    if re.match(r'^(?:\* - )?.+$', next_line) and i + 1 < len(lines) and lines[i + 1].strip():
                        html_lines.append('<ul>')
                        in_list = True
                        current_section = 'bulleted'
                        html_lines.append(f'<li>{next_line.replace("* - ", "").strip()}</li>')
                    else:
                        html_lines.append(f'<p>{next_line}</p>')
            i += 1
            continue

        # Match bullet points with '* - ' or just plain text that might be intended as a bullet
        bullet_match = re.match(r'^(?:\* - )?(.+)$', line)
        if bullet_match and in_list and current_section == 'bulleted':
            item = bullet_match.group(1).strip()
            html_lines.append(f'<li>{item}</li>')
            i += 1
            continue
        elif bullet_match and i > 0 and lines[i-1].startswith('- **'):
            # Start a new list if the previous line was a section heading
            if in_list:
                html_lines.append('</ul>' if current_section == 'bulleted' else '</ol>')
            html_lines.append('<ul>')
            in_list = True
    ...
    formatted_html = ''.join(html_lines)
    print(f"Formatted HTML: {formatted_html}")  # Debug log
...
def voice_chat():
    ...
    if audio_file and allowed_file(audio_file.filename):
        ...
        try:
            ...
            user_language = "ta-IN"  # Assume Tamil for now or detect dynamically if needed
            ...
            if os.path.exists(audio_path):
                ...
                logger.debug(f"Cleaned up uploaded audio file: {audio_path}")
            if os.path.exists(audio_output_path):
                os.remove(audio_output_path)
                logger.debug(f"Cleaned up response audio file: {audio_output_path}")
        ...
        except MultilingualError as e:
            error_msg = str(e)
            if "Invalid file type" in error_msg:
                error_msg = "The uploaded audio file format is not supported. Please upload a WAV or MP3 file and try again."
            elif "STT API returned empty transcription" in error_msg:
                error_msg = "The audio could not be transcribed. It might be empty, too short, or in an unsupported language. Please try recording again with clear speech."
            print(f"Error processing voice chat request: {error_msg}")
            return jsonify({'error': error_msg}), 400
        except Exception as e:
            print(f"Error processing voice chat request: {str(e)}")
            return jsonify({'error': f"An unexpected error occurred. Please try again later. Error: {str(e)}"}), 500
        finally:
            ...
            if 'audio_path' in locals() and os.path.exists(audio_path):
                try:
                    ...
                    logger.error(f"Failed to clean up audio file {audio_path}: {str(e)}")
            if 'audio_output_path' in locals() and os.path.exists(audio_output_path):
                try:
                    os.remove(audio_output_path)
                    logger.debug(f"Cleaned up response audio file: {audio_output_path}")
                except Exception as e:
                    logger.error(f"Failed to clean up audio output file {audio_output_path}: {str(e)}")
    else:
